...
//             return res.status(500).json({ error: 'Firmware is the current, cannot be erased.'})
//         }
//         if (!firmware.filename) {
//             return res.status(500).json({ error: 'Filename is missing for this firmware' });
//         }
...
//             return res.status(500).json({ error: 'Failed to delete firmware file' });
//             }
...
const checkVersion = async (req, res) => {
    try {
        ...
        const response = await FirmwareService.checkVersion(code, version);
...
const getFirmware = async (req, res) => {
  try {
    const { code } = req.body;
    const response = await FirmwareService.getFirmware(code);
    res.status(200).json(response);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
